"""Executor: External-scheduler batch execution + sentinel-driven monitoring.

Runs from **Windows**; all external scheduler commands are issued on CRC's ``<HPC_FILE_LOGIN>``
login node over SSH via WSL's ControlMaster socket. Files are staged on
``<HPC_PATH>`` (AFS-shared across CRC nodes) so external scheduler's
...
Responsibilities
----------------
* Render the external scheduler submit template for each :class:`ExperimentConfig`.
* Stage the submit file + train_config.json + any codegen ``script_path``
  onto CRC via SSH.
* Submit jobs with ``ssh <HPC_FILE_LOGIN> 'cd ... && scheduler_submit ...'``.
* Poll ``scheduler_queue``; map external scheduler JobStatus codes (1=Idle, 2=Running,
...
* GPU ClassAd matches on ``Machine`` hostname, not ``CUDACapability``
  (fix #7), per CRC Dodi.
* AFS: external scheduler executors have no AFS token, so anything under
...
DEFAULT_TEMPLATE = (
    ...
    / "templates" / "scheduler_submit.template"
)
...
# external scheduler JobStatus codes (from the manual): 1=Idle, 2=Running, 3=Removed,
...
class Executor:
    """Submit and monitor CRC external scheduler GPU jobs over SSH."""
    ...
    def __init__(
        ...
        # binary AND cannot find scheduler_submit locally â€” this is mostly
        # useful for offline unit tests.
        self.local_mode = (
            local_mode
            if local_mode is not None
            else (shutil.which("wsl") is None and shutil.which("scheduler_submit") is None)
        )
    ...
    def _remote_codegen_path(self, cfg: ExperimentConfig) -> Optional[str]:
        if not cfg.script_path:
            return None
        # Place codegen-produced files alongside the run so the external scheduler
    ...
    def _stage_and_submit(
        ...
        if cfg.script_path:
            ...
            if not res.ok:
                return res
        # 3. scheduler_submit on the remote.
        submit_cmd = (
            f"cd {shlex.quote(self.remote.project_root)} && "
            f"scheduler_submit {shlex.quote(self._remote_submit_path(cfg))}"
        )
        ...
        if not m:
            return RemoteResult(
                ...
                error="scheduler_submit output did not contain a cluster id",
            )
        return RemoteResult(
    ...
    def _local_submit(self, submit_path: Path) -> RemoteResult:
        exe = shutil.which("scheduler_submit")
        if exe is None:
            return RemoteResult(
                ok=False,
                error="scheduler_submit not on PATH (local_mode) â€” mark failed.",
            )
        try:
            r = subprocess.run(
                [exe, str(submit_path)],
            ...
            return RemoteResult(ok=False, error=f"scheduler_submit timed out: {exc}")
    ...
    def poll(self, handles: list[JobHandle]) -> list[JobHandle]:
        statuses = self._scheduler_queue_map(handles)
        for h in handles:
            if h.status == "failed":
                continue
                ...
                if code == "X" and not (rd / "checkpoint.pt").exists():
                    ...
                    if not (rd / "FAILED").exists():
                        (rd / "FAILED").write_text(
                            json.dumps({"error": "external scheduler removed job"}, indent=2)
                        )
                else:
                    h.status = logical
        return handles
    ...
    def cancel(self, handles: list[JobHandle]) -> None:
        for h in handles:
            if not h.cluster_id:
                continue
            if self.local_mode:
                exe = shutil.which("scheduler_remove")
                if exe is not None:
                    subprocess.run([exe, h.cluster_id], check=False)
            else:
                self._run_remote(
                    f"scheduler_remove {shlex.quote(h.cluster_id)}",
                    timeout=DEFAULT_SSH_TIMEOUT_SEC,
                )

    # ------------------------------------------------------------------
    # external scheduler wrappers
    # ------------------------------------------------------------------
    def _scheduler_queue_map(
        self, handles: Optional[list[JobHandle]] = None,
    ) -> dict[str, str]:
        ids = [h.cluster_id for h in (handles or []) if h.cluster_id]
        if self.local_mode:
            exe = shutil.which("scheduler_queue")
            if exe is None:
                return {}
            cmd = [
                exe, "-nobatch",
            ...
            try:
                ...
                LOGGER.warning("scheduler_queue timed out")
                return {}
            return self._parse_scheduler_queue(res.stdout)
        ...
        cmd = (
            f"scheduler_queue{constraint} -nobatch "
            f"-format '%d.%d ' ClusterId -format '%d ' ProcId "
            f"-format '%s\\n' JobStatus"
        )
        res = self._run_remote(cmd, timeout=DEFAULT_SSH_TIMEOUT_SEC)
        if not res.ok:
            LOGGER.warning("Remote scheduler_queue failed: %s", res.error or res.stderr)
            return {}
        return self._parse_scheduler_queue(res.stdout)

    @staticmethod
    def _parse_scheduler_queue(stdout: str) -> dict[str, str]:
        out: dict[str, str] = {}
    ...
    def _release(self, h: JobHandle) -> None:
        if not h.cluster_id:
            return
        if self.local_mode:
            exe = shutil.which("external_scheduler_release")
            if exe is not None:
                subprocess.run([exe, h.cluster_id], check=False)
            return
        self._run_remote(
            f"external_scheduler_release {shlex.quote(h.cluster_id)}",
            timeout=DEFAULT_SSH_TIMEOUT_SEC,
        )
    ...
    def _requeue(self, h: JobHandle) -> None:
        if h.cluster_id:
            if self.local_mode:
                if shutil.which("scheduler_remove"):
                    subprocess.run(
                        ["scheduler_remove", h.cluster_id], check=False,
                    )
            else:
                self._run_remote(
                    f"scheduler_remove {shlex.quote(h.cluster_id)}",
        ...
        if self.local_mode:
            ...
            submit_cmd = (
                f"cd {shlex.quote(self.remote.project_root)} && "
                f"scheduler_submit {shlex.quote(self._posixify(h.submit_file))}"
            )
            res = self._run_remote(submit_cmd, timeout=DEFAULT_SUBMIT_TIMEOUT_SEC)
            if res.ok:
                m = re.search(r"cluster\s+(\d+)", res.stdout or "")
