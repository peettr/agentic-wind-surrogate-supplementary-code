...
CONDOR_TEMPLATE = PROJECT_ROOT / "templates" / "condor_submit.template"
CONDOR_WRAPPER = PROJECT_ROOT / "templates" / "condor_wrapper.sh"
...
class JobHandle:
    ...
    scheduler: str = "condor"  # "condor" or "sge"
    submit_file: str = ""
    results_dir: str = ""
...
class Executor:
    ...
    # â”€â”€ Path 1: GPU training (Condor) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def submit_gpu_train(
        ...
        """Submit a GPU training job via Condor (V3 pattern)."""
        ...
        # Render Condor submit template
        template_text = CONDOR_TEMPLATE.read_text(encoding="utf-8")
        ...
        # Idempotency guard: if a previous runner died after condor_submit but
        ...
        # Keep the remote condor_q itself bounded.  CRC condor_q can occasionally
        ...
        check_cmd = (
            "timeout 360s condor_q lhu1 -nobatch "
        ...
        if existing_cluster and existing_cluster[0].isdigit():
            ...
            LOGGER.info("Condor job already exists for %s -> cluster %s", exp_id, handle.cluster_id)
            return handle
        ...
        # Shared filesystem visibility barrier.  Condor jobs can start within a
        ...
        # 4. condor_submit (must activate conda first so getenv=True passes conda env)
        conda_activate = (
        ...
        submit_cmd = (
            ...
            f"condor_submit {shlex.quote(remote_submit)}"
        )
        res = self.run_remote(submit_cmd, timeout=DEFAULT_SUBMIT_TIMEOUT)
        if not res.ok:
            handle.status = "failed"
            (local_results / "FAILED").write_text(
                json.dumps({"error": f"condor_submit failed: {res.error}"}, indent=2))
            return handle
        ...
        if not m:
            handle.status = "failed"
            (local_results / "FAILED").write_text(
                json.dumps({"error": "no cluster_id in condor_submit output"}, indent=2))
            return handle

        handle.cluster_id = m.group(1)
        LOGGER.info("Condor submitted %s -> cluster %s", exp_id, handle.cluster_id)
        return handle
    ...
    def poll_condor(self, handles: list[JobHandle]) -> dict[str, str]:
        """Poll Condor job status. Returns {cluster_id: status_code}."""
        ids = [h.cluster_id for h in handles if h.cluster_id and h.scheduler == "condor"]
        if not ids:
            return {}
        constraint = " || ".join(f"ClusterId=={i}" for i in ids)
        # Bound the remote condor_q call itself. If CRC's scheduler query or
        ...
        cmd = (
            f"timeout 360s condor_q -constraint {shlex.quote(constraint)} -nobatch "
        ...
        for line in (res.stdout or "").splitlines():
            ...
            if len(parts) >= 2:
                cid = parts[0].split(".")[0]
                out[cid] = _CONDOR_STATUS.get(parts[-1], "I")
        return out

    def condor_hold_reason(self, cluster_id: str) -> str:
        """Fetch bounded Condor hold reason text for classifier evidence."""
        if not cluster_id:
            return ""
        cmd = (
            "timeout 30s condor_q " + shlex.quote(cluster_id) +
    ...
    def tail_condor_logs(self, handle: JobHandle, n: int = 30) -> str:
        """Tail condor.out + condor.err for crash detection."""
        rd = handle.remote_results_dir
        if not rd:
            return ""
        exp_dir = f"{rd}/{handle.experiment_id}" if not rd.endswith(handle.experiment_id) else rd
        ...
        files = [exp_dir + "/condor.log", exp_dir + "/condor.out", exp_dir + "/condor.err", exp_dir + "/train.log"]
        cmd = " ; ".join(
    ...
    def poll_handles(self, handles: list[JobHandle]) -> list[JobHandle]:
        ...
        # Split by scheduler
        condor_handles = [h for h in handles if h.scheduler == "condor"]
        sge_handles = [h for h in handles if h.scheduler == "sge"]

        # Poll Condor
        condor_statuses = self.poll_condor(condor_handles)
        for h in condor_handles:
            if h.status in ("completed", "failed"):
                continue
            code = condor_statuses.get(h.cluster_id or "")
            if code:
                logical = _CONDOR_LOGICAL.get(code, h.status)
                if code == "X":
                    ...
                    h.status = logical
                    if logical == "held":
                        reason = self.condor_hold_reason(h.cluster_id or "")
                        if reason:
                            h.log_tail = (h.log_tail + "\n" if h.log_tail else "") + "CONDOR_HOLD_REASON: " + reason
            else:
                ...
                if sentinel:
                    ...
                    # runner ever observed it as running. Condor then removes it
                    # from the queue, no FINISHED/FAILED sentinel is present,
                    # and status can remain "submitted" forever. Inspect the
                    # Condor logs immediately for terminal/crash evidence.
                    logs = self.tail_condor_logs(h, 20)
    ...
    def cancel_condor(self, handles: list[JobHandle]) -> None:
        for h in handles:
            if h.cluster_id and h.scheduler == "condor":
                self.run_remote(f"timeout 20s condor_rm {shlex.quote(h.cluster_id)} || true", timeout=30)
