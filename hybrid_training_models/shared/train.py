...
def _resolve_model_cls(cfg: TrainConfig):
    ...
    if cfg.script_path:
        ...
        raise AttributeError(
            f"No torch.nn.Module class named {cfg.arch_name!r} in {cfg.script_path}"
        )
    generated_path = _HERE.parent / "models" / "generated" / f"{cfg.arch_name}.py"
    if generated_path.is_file():
        module = _load_external_module(generated_path)
        if hasattr(module, cfg.arch_name):
            return getattr(module, cfg.arch_name)
        raise AttributeError(
            f"No torch.nn.Module class named {cfg.arch_name!r} in {generated_path}"
        )
...
def _input_channels_for_features(input_features: str) -> int:
    """Return model input channels implied by the locked feature contract."""
    if input_features == "height_sdf":
        return 2
    if input_features == "height_sdf_normal":
        return 3
    return 1


def _model_kwargs_for_cfg(model_cls: type, cfg: TrainConfig) -> dict:
    """Prepare constructor kwargs, including feature-derived input channels.

    Planner configs specify feature augmentation at TrainConfig.input_features,
    not inside arch_kwargs.  Without this bridge, SDF runs build a 1-channel
    model and then feed 2/3-channel tensors, producing Conv2d group/input
    channel errors during smoke.  Inject in_channels only when the model can
    accept it or supports **kwargs, preserving existing height-only behavior.
    """
    kwargs = dict(cfg.arch_kwargs or {})
    expected_in = _input_channels_for_features(cfg.input_features)
    if expected_in != 1 and "in_channels" not in kwargs:
        try:
            sig = inspect.signature(model_cls.__init__)
            params = sig.parameters
            accepts_kwargs = any(p.kind == inspect.Parameter.VAR_KEYWORD for p in params.values())
            if "in_channels" in params or accepts_kwargs:
                kwargs["in_channels"] = expected_in
        except (TypeError, ValueError):
            # If introspection fails, keep the old kwargs and let the model raise
            # a clear constructor/forward error.
            pass
    return kwargs
...
def train(cfg: TrainConfig) -> dict:
    ...
    # Extra training hyperparams from variant.training (parse FIRST)
    training_extras = (cfg.arch_kwargs or {}).get("training", {}) or {}
    ...
    model_kwargs = _model_kwargs_for_cfg(model_cls, cfg)
    ...
    params_million = model_params / 1_000_000.0
    LOGGER.info(
        "Device=%s GPU=%s params=%d trainable_params=%d params_million=%.6f",
        device, gpu, model_params, trainable_params, params_million,
    )
...
def eval_only(cfg: TrainConfig, model_path: str | Path, split: str = "holdout") -> dict:
    ...
    metrics = evaluator.evaluate(model, split=split, seed=cfg.seed, device=device)
    metrics["model_params"] = model_params
    metrics["trainable_params"] = trainable_params
    metrics["params_million"] = model_params / 1_000_000.0
    return metrics
...
if __name__ == "__main__":
    ...
    if _args.eval_holdout:
        ...
        metrics = {
            ...
            "trainable_params": out.get("trainable_params"),
            "params_million": out.get("params_million"),
            "status": "ok",
            "val_metrics": {
                "r2_median": float("nan"), "r2_mean": float("nan"),
