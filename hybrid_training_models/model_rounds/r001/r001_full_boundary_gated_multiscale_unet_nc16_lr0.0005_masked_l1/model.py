...
class boundary_gated_multiscale_unet(nn.Module):
    ...
    class Gate(nn.Module):
        def __init__(self, skip_ch, dec_ch):
            super().__init__()
            ...
            self.edge_proj = nn.Conv2d(1, skip_ch, kernel_size=1, padding=0)
    ...
    def __init__(self, in_channels=1, out_channels=1, n_c=16, depth=6):
        ...
        for i in range(depth - 2, -1, -1):
            self.gates.append(self.Gate(channels[i], channels[i + 1]))
    ...
    def _edge_map(self, x):
        ...
        edge = gx.mean(dim=1, keepdim=True) + gy.mean(dim=1, keepdim=True)
    ...
    def forward(self, x):
        ...
        if out_valid.shape[1] != out.shape[1]:
            out_valid = out_valid.any(dim=1, keepdim=True).expand_as(out)
