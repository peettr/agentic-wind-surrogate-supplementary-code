#!/bin/bash
# preflight_check.sh — Run before every Condor job submission
...
if [ ! -f "$SUBMIT_FILE" ]; then
    echo "❌ FAIL: Submit file not found: $SUBMIT_FILE"
    exit 1
fi
echo "✅ Submit file exists"

# 2. Parse all run names from submit file
NAMES=$(grep "^Name = " "$SUBMIT_FILE" | sed 's/Name = //' | sort)
N_TOTAL=$(echo "$NAMES" | wc -l)
echo "✅ Found $N_TOTAL runs in submit file"
...
if [ $MISSING_CONFIG -eq 0 ]; then
    echo "✅ All $N_TOTAL train_config.json present"
fi
...
for name in $NAMES; do
    for ext in out err log; do
        ...
        if [ -f "$F" ]; then
            # Check writable
            if [ ! -w "$F" ]; then
                echo "⚠️  FIX: Removing unwritable $F"
                rm -f "$F" 2>/dev/null || true
                PERM_ISSUES=$((PERM_ISSUES + 1))
                FIXES=$((FIXES + 1))
            else
                # Stale file from previous run — truncate
                echo "⚠️  FIX: Truncating stale $F"
                > "$F"
    ...
    if [ -d "$RUNDIR" ] && [ ! -w "$RUNDIR" ]; then
        echo "⚠️  FIX: chmod run dir $RUNDIR"
...
if [ $PERM_ISSUES -eq 0 ] && [ $STALE_FILES -eq 0 ]; then
    echo "✅ No permission/stale file issues"
fi
    ...
    if [ -f "$CFG" ]; then
        for field in experiment_id seed epochs lr batch_size arch_name loss_name data_dir results_dir split_manifest_path; do
            if ! python3 -c "import json; c=json.load(open('$CFG')); assert '$field' in c or '$field' in c.get('arch_kwargs',{}).get('training',{})" 2>/dev/null; then
                echo "❌ BAD CONFIG: $name missing field '$field'"
        ...
        if ! python3 -c "import json; c=json.load(open('$CFG')); assert c.get('compute_r2') == True" 2>/dev/null; then
            echo "⚠️  WARNING: $name compute_r2 not True"
        fi
    fi
done
if [ $BAD_CONFIG -eq 0 ]; then
    echo "✅ All configs have required fields"
fi
...
if [ -n "$DATA_DIR" ]; then
    for f in all_data.pt split_manifest.json; do
        if [ -f "$DATA_DIR/$f" ]; then
            echo "✅ Data file exists: $DATA_DIR/$f"
        else
            echo "❌ MISSING DATA: $DATA_DIR/$f"
            ISSUES=1
        fi
    done
fi
...
if [ $ISSUES -eq 0 ]; then
    ...
    echo "❌ Issues found — review before submitting"
    exit 1
fi
