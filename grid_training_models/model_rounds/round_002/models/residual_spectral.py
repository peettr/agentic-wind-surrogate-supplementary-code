"""Generated standalone Auto V5 model for residual_spectral.
...
class SpectralBottleneck(nn.Module):
    ...
    def __init__(self, ch: int, modes: int = 6) -> None:
        super().__init__()
        self.modes = modes
        scale = 1.0 / (ch * ch)
...
class ResidualSpectralNet(BaseSurrogate):
    ...
    def __init__(self, n_c: int = 48, depth: int = 4, modes: int = 6) -> None:
        super().__init__()
        self.depth = depth
...
class Model(ResidualSpectralNet):
