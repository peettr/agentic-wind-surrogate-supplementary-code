...
This document defines the main grid benchmark protocol derived from baseline_source findings. It separates hard rules from soft rules and is written as an operational instruction set for agents and humans.

The goal is to keep grid comparable, auditable, and compute-efficient by fixing the task and evaluation contracts while focusing search on the variables that mattered in baseline_source.

---

## Hard Rules, mandatory

These rules are mandatory for any run that will be included in the main grid benchmark. If a run violates any item below, it must be labeled exploratory and excluded from the main leaderboard.

### Task and data contract

1. All main benchmark runs must use the fixed 640Ã—640 task contract.
...
8. The primary ranking metric is per-case median RÂ².
9. Each official result must also report MAE, global RÂ², wall time, and peak VRAM.
...
These are default policies for the main grid workflow. They may be changed only with explicit justification recorded in the experiment note or report.
...
- `loss âˆˆ {masked_l1, masked_l1_gradient}`
- `lr âˆˆ {1e-3, 5e-4}`
- `ema âˆˆ {None, 0.999}`
- optional `scheduler âˆˆ {None, cosine}`
...
Use the following instructions when executing grid experiments.

### Mandatory execution rules

1. Use the fixed 640Ã—640 task contract.
