...
**Reference Baseline for orthogonal exploratory sweep: best_val = 0.0997 (New L40S, 150ep, val=95)**
...
|---|---|---|---|
| Loss | masked_l1 (NaN+building mask) | MaskedL1Loss (same) | âœ… |
| lr | 1e-3 | 1e-3 | âœ… |
| Optimizer | Adam (no wd) | Adam (no wd) | âœ… |
| Scheduler | None | None | âœ… |
| Model | UNetLu7Level(n_c=16) | UNetV2Baseline(n_c=16) | âœ… |
| Seed | 1 | 1 | âœ… |
| Data | full_masked_640 (878 patches) | all_data.pt (same data, concatenated) | âœ… |
...
- **train.py was modified at 11:12** (val+ho â†’ val-only), but this does NOT affect train loss
