...
def _resolve_model_cls(cfg: TrainConfig):
    ...
    if cfg.script_path:
        ...
        # Prefer an attribute matching arch_name; else take first torch.nn.Module subclass.
...
def train(cfg: TrainConfig) -> dict:
    ...
    if device.type == "cuda":
        ...
        if bs == 0:
            ...
            _write_metrics(results_dir, cfg, val_metrics=None, holdout_metrics=None,
                           status="failed", error="OOM at batch=8", wall_time=0,
                           peak_vram=0, device=str(device))
            return
