...
For each seed:
  ...
  4. Compute R² in raw wind speed domain
  5. Compare with Lu's per-case R²
"""
...
def get_lu_metrics(seed):
    """Get Lu's per-case R² and global R²."""
    lu_test = pd.read_csv(REF_DIR / f"metrics_in_test_set_seed{seed}.csv")
...
def restore_and_r2(cn, pred_all):
    ...
    if n_patches != expected_n:
        print(f"  ERROR: {cn} has {n_patches} patches but "
              f"DataFormatter expects {expected_n} — SKIPPING")
        return float('nan'), float('nan'), None, None
    ...
    # Returns (N, H_raw, W_raw) — each patch restored to full domain
    restored = formatter.restore_raw_output_data(pred_input)

    # Average across patches (NaN in non-covered regions → nanmean blends)
    pred_raw = np.nanmean(restored, axis=0)
    truth = combined

    # Handle shape mismatch (should NOT happen — log warning if it does)
    if truth.shape != pred_raw.shape:
        if truth.shape == pred_raw.T.shape:
            print(f"  WARNING: {cn} needed transpose — "
    ...
    # R² on valid (non-building, finite) pixels
    # Buildings encoded as negative values in combined: truth < 0 → building
...
def eval_seed(seed, pred_np):
    ...
    if n_fail > 0:
        print(f"  WARNING: {n_fail} cases failed restore — "
              f"global R² computed on {n_ok}/{len(test_cases)} cases only")
    ...
    print(f"  RAW Global R² = {global_r2:.4f}, MAE = {global_mae:.4f}")
    print(f"  Lu Global R² = {lu_global_r2:.4f}")
...
print("=" * 80)
print(f"{'Seed':>5} | {'Our R²':>8} | {'Lu R²':>8} | {'Δ':>8} | "
...
if results:
    ...
    print(f"Mean: Our={a:.4f}±{sa:.4f}, Lu={b:.4f}±{sb:.4f}, Δ={a-b:+.4f}")
